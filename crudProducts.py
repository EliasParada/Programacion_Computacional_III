...
class crud:
    ...
    def show_limits(self, id):
        try:
            sql = "SELECT prt_producto.prt_id, prt_producto.prt_name, prov_proveedor.prov_id, prov_proveedor.prov_name, prt_producto.prt_createdate, prt_producto.prt_expirationdate, cat_categoria.cat_id, cat_categoria.cat_name, prt_producto.prt_cost, prt_producto.prt_photo FROM prt_producto INNER JOIN cat_categoria ON prt_producto.cat_id = cat_categoria.cat_id INNER JOIN prov_proveedor ON prt_producto.prov_id = prov_proveedor.prov_id LIMIT %s, %s"
            max_id = id-20
            if max_id < 0:
                max_id = 0
