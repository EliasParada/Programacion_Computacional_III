...
#Crear las etiquetas de los productos
...
# Predecir 1 imagen del dataset de prueba
for imagen, etiqueta in datos_prueba.take(1):
    prediccion_probabilidad = modelo.predict(imagen)
    prediccion = tf.math.argmax(prediccion_probabilidad, axis=1)
    print("Etiqueta de la imagen: ", tags[etiqueta[0]])
    print("Prediccion: ", tags[prediccion.numpy()[0]])
...
class localServer(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        data = parse.unquote(body.decode('utf-8'))
        print(data)
